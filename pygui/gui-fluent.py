...
class SettingInterface(SmoothScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self.vLayout.setContentsMargins(36, 24, 36, 36)
        self.vLayout.setSpacing(16)

        self.vLayout.addWidget(TitleLabel("设置"))
        self.vLayout.addSpacing(4)

        # ---- 常规设置组 ----
        self.vLayout.addWidget(SubtitleLabel("常规"))
        self.vLayout.addWidget(ComboSettingCard(
            title="启动模式",
            desc="设置软件启动时的行为",
        ...
        self.vLayout.addWidget(SwitchSettingCardCustom(
        ...
        # ---- 操作按钮组 ----
        self.vLayout.addWidget(SubtitleLabel("操作"))

        # 检查更新
        update_card = CardWidget()
        update_layout = QHBoxLayout(update_card)
        update_layout.setContentsMargins(16, 12, 16, 12)
        update_layout.addWidget(IconWidget(FIF.UPDATE, update_card))
        update_layout.addSpacing(12)
        update_text = QVBoxLayout()
        update_text.addWidget(BodyLabel("检查更新"))
        update_text.addWidget(CaptionLabel("检查软件更新"))
        update_layout.addLayout(update_text)
        update_layout.addStretch()
        update_btn = PushButton("检查", update_card)
        update_btn.clicked.connect(self._check_update)
        update_layout.addWidget(update_btn)
        self.vLayout.addWidget(update_card)

        # 打开日志文件夹
        open_log_btn_card = CardWidget()
        open_log_layout = QHBoxLayout(open_log_btn_card)
        open_log_layout.setContentsMargins(16, 12, 16, 12)
        open_log_layout.addWidget(IconWidget(FIF.FOLDER, open_log_btn_card))
        ...
        self.vLayout.addWidget(open_log_btn_card)

        # 使用经典界面
        oldui_card = CardWidget()
        oldui_layout = QHBoxLayout(oldui_card)
        oldui_layout.setContentsMargins(16, 12, 16, 12)
        oldui_layout.addWidget(IconWidget(FIF.HISTORY, oldui_card))
        oldui_layout.addSpacing(12)
        oldui_text = QVBoxLayout()
        oldui_text.addWidget(BodyLabel("使用经典界面"))
        oldui_text.addWidget(CaptionLabel("切换到经典用户界面"))
        oldui_layout.addLayout(oldui_text)
        oldui_layout.addStretch()
        oldui_btn = PushButton("切换", oldui_card)
        oldui_btn.clicked.connect(lambda: SeewoKiller_run.OldUI().start())
        oldui_layout.addWidget(oldui_btn)
        self.vLayout.addWidget(oldui_card)

        self.vLayout.addSpacing(8)

        # ---- 高级设置组 ----
        self.vLayout.addWidget(SubtitleLabel("高级"))
        ...
        self.vLayout.addWidget(freeze_help_card)
        ...
        self.vLayout.addWidget(cmd_help_card)
        ...
        self.vLayout.addWidget(fastboot_card)

        self.vLayout.addSpacing(8)

        # ---- 关于 ----
        self.vLayout.addWidget(SubtitleLabel("关于"))
    ...
    def _check_update(self):
        self._update_window = show_update_dialog()
...
"""
    ...
    def add_dev_options(self):
        ...
        self.vLayout.insertWidget(about_subtitle_index, dev_subtitle)
        about_subtitle_index += 1
        ...
        com_btn.clicked.connect(lambda: MessageBox("提示", "COM 接口释放操作已完成（模拟）", self).exec())
        com_layout.addWidget(com_btn)
        self.vLayout.insertWidget(about_subtitle_index, com_card)
